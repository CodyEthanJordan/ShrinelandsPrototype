...
namespace UnityEditor
{
    ...
    internal class WallRuleTileEditor : Editor
    {
        ...
        private WallRuleTile tile { get { return (target as WallRuleTile); } }
        ...
        public void OnEnable()
        {
            if (tile.m_TilingRules == null)
                tile.m_TilingRules = new List<WallRuleTile.TilingRule>();

            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(WallRuleTile.TilingRule), true, true, true, true);
        ...
        {
            WallRuleTile.TilingRule rule = tile.m_TilingRules[index];
        ...
        {
            WallRuleTile.TilingRule rule = new WallRuleTile.TilingRule();
            rule.m_Output = WallRuleTile.TilingRule.OutputSprite.Single;
        ...
        public override void OnInspectorGUI()
        {
            ...
            tile.MatchingTile = EditorGUILayout.TextField(tile.MatchingTile);
            ...
            EditorGUI.BeginChangeCheck();
            var baseFields = typeof(WallRuleTile).GetFields().Select(field => field.Name);
        ...
        internal virtual void RuleOnGUI(Rect rect, int arrowIndex, int neighbor)
        {
            switch (neighbor)
            {
                case WallRuleTile.TilingRule.Neighbor.DontCare:
                    break;
                case WallRuleTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[arrowIndex]);
                    break;
                case WallRuleTile.TilingRule.Neighbor.NotThis:
                    GUI.DrawTexture(rect, arrows[9]);
                    break;
                case WallRuleTile.TilingRule.Neighbor.Wall:
        ...
        internal virtual void RuleTransformOnGUI(Rect rect, WallRuleTile.TilingRule.Transform ruleTransform)
        {
            switch (ruleTransform)
            {
                case WallRuleTile.TilingRule.Transform.Rotated:
                    GUI.DrawTexture(rect, autoTransforms[0]);
                    break;
                case WallRuleTile.TilingRule.Transform.MirrorX:
                    GUI.DrawTexture(rect, autoTransforms[1]);
                    break;
                case WallRuleTile.TilingRule.Transform.MirrorY:
                    GUI.DrawTexture(rect, autoTransforms[2]);
                    break;
            }
        }

        internal void RuleNeighborUpdate(Rect rect, WallRuleTile.TilingRule tilingRule, int index)
        {
        ...
        internal void RuleTransformUpdate(Rect rect, WallRuleTile.TilingRule tilingRule)
        {
            if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
            {
                tilingRule.m_RuleTransform = (WallRuleTile.TilingRule.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), 4);
                GUI.changed = true;
                Event.current.Use();
            }
        }
        ...
        internal virtual void RuleMatrixOnGUI(WallRuleTile tile, Rect rect, WallRuleTile.TilingRule tilingRule)
        {
        ...
        internal static void SpriteOnGUI(Rect rect, WallRuleTile.TilingRule tilingRule)
        {
        ...
        internal static void RuleInspectorOnGUI(Rect rect, WallRuleTile.TilingRule tilingRule)
        {
            float y = rect.yMin;
            EditorGUI.BeginChangeCheck();
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
            tilingRule.m_RuleTransform = (WallRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
            ...
            tilingRule.m_Output = (WallRuleTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == WallRuleTile.TilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == WallRuleTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (WallRuleTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != WallRuleTile.TilingRule.OutputSprite.Single)
            {
        ...
        [Serializable]
        class WallRuleTileRuleWrapper
        {
            [SerializeField]
            public List<WallRuleTile.TilingRule> rules = new List<WallRuleTile.TilingRule>();
        }
